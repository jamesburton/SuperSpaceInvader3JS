...
- [ ] **AUD-01**: Player hears a looping synthwave BGM track during gameplay without audible gaps
...
- [ ] **AUD-05**: Audio plays on first user interaction without requiring page refresh (AudioContext unlock)
- [ ] **AUD-06**: Player can mute/unmute and adjust master volume from pause menu
- [ ] **AUD-07**: Volume preference persists across sessions

### Gamepad

- [ ] **PAD-01**: Player can move ship with left analog stick with deadzone filtering
- [ ] **PAD-02**: Player can shoot with A button and pause with Start button
- [ ] **PAD-03**: Player can navigate all menus and shop UI with gamepad
- [ ] **PAD-04**: Game displays notification on gamepad connect/disconnect
- [ ] **PAD-05**: Game gracefully falls back to keyboard when gamepad disconnects mid-game

### Ship Skins

- [ ] **SKIN-01**: Player can view and select from 3-4 distinct ship geometry shapes in a selection UI
- [ ] **SKIN-02**: Player can choose from 6 color variants per ship shape
- [ ] **SKIN-03**: Ship skins are purchasable with SI$ in the meta shop
- [ ] **SKIN-04**: Selected skin and color persist across sessions
...
- [ ] **CRT-01**: Player can unlock CRT Tier 1 "HIGH-DEF 2003" (light scanlines) in meta shop
- [ ] **CRT-02**: Player can unlock CRT Tier 2 "CONSUMER 1991" (moderate scanlines + chromatic aberration)
- [ ] **CRT-03**: Player can unlock CRT Tier 3 "ARCADE 1983" (heavy scanlines + strong chromatic aberration)
- [ ] **CRT-04**: Player can adjust CRT effect intensity with a slider
- [ ] **CRT-05**: CRT effect updates in real-time as settings change (no restart required)
